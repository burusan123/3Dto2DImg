...
- **マウス中ボタンドラッグによるカメラパン**
- **マウス右クリックドラッグによるカメラビュー回転（UE5スタイル）**
...
4. 以下のキーを使用して3D空間内を移動・操作できます：
   ...
   - **マウス右ボタン（右クリックドラッグ）**: カメラビューを回転（UE5スタイル）
...
- **enabled**: 目盛りの表示を有効/無効にします
- **interval**: 目盛りの間隔（mm単位）。デフォルトは300mm
  - 例: 100mm間隔にする場合は`interval: 100`
- **major_interval**: 主目盛りの間隔（通常の目盛りの何本毎に太い線を引くか）
  - デフォルト: 5（5本毎 = 1500mm毎）
- **label_show**: 数値ラベルの表示/非表示
- **label_font_scale**: ラベルのフォントサイズ（0.1〜1.0程度）

### 利用シーン

- **レイアウト確認**: 3Dビューと平面図を比較して、家具の配置を確認
- **寸法確認**: 目盛りを使って家具間の距離や配置位置を正確に把握
...
- `camera.mouse_view_rotation.sensitivity`: 右クリックドラッグの回転感度（度/ピクセル）
- `camera.mouse_view_rotation.invert_x`: 水平回転（Yaw）の反転（true/false）
- `camera.mouse_view_rotation.invert_y`: 垂直回転（Pitch）の反転（true/false）
- `camera.mouse_view_rotation.min_pitch`: Pitch の最小値（例：70度）
- `camera.mouse_view_rotation.max_pitch`: Pitch の最大値（真下を見る、90度）
...
#### 右クリックドラッグ設定の詳細（UE5スタイル）

マウス右クリックドラッグでのカメラビュー回転の動作をカスタマイズできます：

```yaml
camera:
  mouse_view_rotation:
    sensitivity: 0.2  # 回転の感度（度/ピクセル）
    invert_x: false   # 水平回転を反転
    invert_y: false   # 垂直回転を反転
    min_roll: -85     # roll の最小値（真上を見る限界）
    max_roll: 265     # roll の最大値（真下を見る限界）
```

**設定パラメータ:**
- `sensitivity`: マウス移動量に対する回転角度の倍率（デフォルト: 0.2度/ピクセル）
  - 大きくすると少しのマウス移動で大きく回転する
  - 小さくすると精密な操作が可能
- `invert_x`: 水平回転（Yaw）の反転
  - `false` (デフォルト): マウス右 → 右を向く、マウス左 → 左を向く
  - `true`: マウス右 → 左を向く、マウス左 → 右を向く
- `invert_y`: 垂直回転（Pitch）の反転
  - `false` (デフォルト): マウス下 → 下を見る、マウス上 → 上を見る
  - `true`: マウス下 → 上を見る、マウス上 → 下を見る
- `min_pitch` / `max_pitch`: カメラが見上げる/見下ろす角度の制限
  - Unreal Engine 5と同様のFPS/TPSスタイルのカメラ操作を実現

## 座標系と回転の理解（UE5互換）

### ワールド座標系

このアプリケーションはUnreal Engine 5と同じ右手座標系を使用しています：
...
| **パラメータ** | **回転軸** | **動作** | **特殊な値** |
|-------------|----------|---------|-----------|
| **Roll** | X軸周り（前方軸） | 左右に傾く | 正=右に傾く, 負=左に傾く |
| **Pitch** | Y軸周り（右方軸） | 上下を見る | 正=下を見る, 負=上を見る, 90度=真下（トップビュー） |
| **Yaw** | Z軸周り（上方軸） | 左右を向く | 正=左を向く, 負=右を向く |
...
camera:
  ...
  initial_rotation:
    roll: 0      # カメラの左右傾きなし
    pitch: 90    # Y軸周りに90度 = 真下を向く
    yaw: 0       # 水平回転なし
```

**説明**: カメラを真上に配置し、Pitch=90度でY軸周りに回転することで真下を向けます。これにより、床面が正しく見えるトップビューになります。

### 他の視点の例

#### 斜め上から見下ろす
```yaml
initial_position: {x: 250, y: 250, z: 500}
initial_rotation: {roll: 0, pitch: -45, yaw: 0}
```
...
initial_rotation: {roll: 0, pitch: 0, yaw: 90}
```
